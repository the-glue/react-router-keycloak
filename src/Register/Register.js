...
class Register extends React.Component {
  ...
  componentDidMount() {
    const { location, onSuccess, onFailure, clientId } = this.props;
      ...
      .then((authenticated) => {
        if (authenticated) {
          ...
          if (location.state && location.state.redirectTo) {
            // Store the current path in session storage before leaving the application to register on keycloak server
            window.sessionStorage.setItem(
              'keycloak-react-router:redirectTo',
              JSON.stringify(location.state.redirectTo)
            );
          }
          // Redirect to keycloak register page
          if (clientId) {
